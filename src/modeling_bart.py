# codes from huggingface/transformers
...
class MixoutBartEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = mixout(hidden_states, p=self.dropout, training=self.training)
        ...
        hidden_states = mixout(hidden_states, p=self.activation_dropout, training=self.training)
...
class MixoutBartDecoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        super().__init__()
        self.embed_dim = config.d_model
...
class BartEncoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([MixoutBartEncoderLayer(config) for _ in range(config.encoder_layers)])
...
class BartDecoder(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([MixoutBartDecoderLayer(config) for _ in range(config.decoder_layers)])
...
class MixoutBartForConditionalGeneration(BartPretrainedModel):
    base_model_prefix = "model"
